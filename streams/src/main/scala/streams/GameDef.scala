...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
     ...
     *
     * origen: def neighbors: List[(Block, Move)] = ???
     *
     */
     ...
     *
     * origen: def legalNeighbors: List[(Block, Move)] = ???
     */
    def legalNeighbors: List[(Block, Move)] = neighbors.filter{ case (block, move) => block.isLegal}
    /*
    1st version:
    def legalNeighbors: List[(Block, Move)] = neighbors.filter( blockMove => blockMove._1.isLegal)
    */

    /**
     * Returns `true` if the block is standing.
      *
      * origen:def isStanding: Boolean = ???
     */
     ...
     * Returns `true` if the block is entirely inside the terrain.
      *
      * origen: def isLegal: Boolean = ???
     */
    def isLegal: Boolean = terrain(b1) && terrain(b2)
    //Currently this isLegal function returns true when the Block is inside a valid position (not a -) inside the terrain
    ///TODO: This functions is well implemented?
  }
}
